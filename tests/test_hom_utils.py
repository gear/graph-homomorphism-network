...
def test_cycle_list():
    cyclel2 = path_list(size=4, to_homlib=True, num_loops=1)
    assert 12 == len(cyclel2)


def test_path_list():
    pathl2 = path_list(size=2, to_homlib=True, num_loops=0)
    assert 1 == len(pathl2)


def test_tree_list():
    treel2 = tree_list(size=2, to_homlib=True, num_loops=0)
    assert 1 == len(treel2)
    treel2 = tree_list(size=2, to_homlib=True, num_loops=1)
    assert 3 == len(treel2)
    treel6 = tree_list(size=6, to_homlib=False, num_loops=0)
    assert 13 == len(treel6)
    treel6 = tree_list(size=6, to_homlib=False, num_loops=1)
