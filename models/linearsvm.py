...
parser = argparse.ArgumentParser('Linear SVC with homomorphism profile.')
# Data loader
...
# Hyperparams for Linear SVC
parser.add_argument("--C", type=float, help="SVC's C parameter.", default=1.0)
parser.add_argument("--penalty", type=str, help="l1 or l2.", default="l2")
parser.add_argument("--loss", type=str, help="LinearSVC loss", 
                    default="squared_hinge")
parser.add_argument("--max_iter", type=int, help="Max iteration", default=10000)
parser.add_argument("--tol", type=float, help="Tolerance for early stopping",
                    default=1e-2)
# Misc
...
penalty = ['l1']
loss = ['squared_hinge']
Cs = [0.01, 0.1, 1.0, 2.0, 10.0, 100.0]
class_weight = ['balanced']
max_iter = [100000]
dual = [False]
tol = [1e-2]
param_grid = {'C': Cs, 'penalty': penalty, 'class_weight': class_weight,
              'loss': loss, 'dual': dual, 'tol': tol, 'max_iter': max_iter}
...
if __name__ == "__main__":
    ...
    for j in tqdm(range(args.num_run)):
        ...
        for train_idx, test_idx in skf.split(X, y): 
            ...
            if args.grid_search:
                grid_search = GridSearchCV(LinearSVC(), param_grid, iid=False, 
                                           cv=args.gs_nfolds, n_jobs=8)
                grid_search.fit(X_train,y_train)
                clf = LinearSVC(**grid_search.best_params_)
            else:
                clf = LinearSVC(penalty=args.penalty, loss=args.loss, C=args.C,
                                class_weight="balanced", max_iter=args.max_iter,
                                dual=False, tol=args.tol)
    ...
    print("Time for Linear SVC: {:.2f} sec".format(svm_time/(args.num_run*\
